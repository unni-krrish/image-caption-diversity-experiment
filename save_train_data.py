...
imgdir = os.path.join(cocodir, "val2014")
transfer_values = os.path.join(cocodir, "/B0_transfer_vals_val14.pkl")
annots_json = os.path.join(cocodir, "/annotations/captions_val2014.json")
...
# Get all the annotations from the COCO annotation file
down_and_extract(annots_url, annots_file, cocodir)

# Process the captions and tokenize it.
# Transfer values and Token list will later be loaded in training.
with open(annots_json, 'r') as f:
    targ = f.read()
...
# Flattened captions are now tokenized
